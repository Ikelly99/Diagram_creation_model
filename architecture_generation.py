...
with col2:
    if buttom_check and option == 'AWS' and text_input:
        page_response(tech = "AWS", text_input=text_input, option="AWS", dic_tech=dic_tech)

    elif buttom_check and option == 'GCP' and text_input:
       page_response(tech = "GCP", text_input = text_input, option="GCP", dic_tech = dic_tech)
        ...
        if analyze_prompt(text_input) == "Allowed" and cla == 1:
            ...
            message = f"Your message was flagged as malicious: {cla}"
            st.write(message)
        page_response(tech = "Azure", text_input = text_input, option = "Azure", dic_tech = dic_tech)
